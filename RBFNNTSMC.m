function [u,S,e,eDot] = ...
    RBFNNTSMC(wStar,x,n,XD,XDotD,XDoubleDotD,b, bPrime, Landa,LandaPrime,Eta,K,FHAT_ST)
    ...
    TrackingPartX = Kf*xDot/m+XDoubleDotD(1)-Eta(1)*Sx-K(1)*tanh(Sx)-FHAT_X;
    ...
    TrackingPartY = Kf*yDot/m+XDoubleDotD(2)-Eta(2)*Sy-K(2)*tanh(Sy)-FHAT_Y;
    ...
    TrackingPartZ = Kf*zDot/m+XDoubleDotD(3)-Eta(3)*Sz-K(3)*tanh(Sz)-FHAT_Z;
    ...
    TrackingPartPhi = -XDoubleDotD(4)+((Iy-Iz)/Ix)*thetaDot*psiDot+...
        JTP*thetaDot*wStar/Ix-Kt*L*phiDot/Ix+Eta(4)*Sphi+K(4)*tanh(Sphi)+FHAT_PHI;
    ...
    TrackingPartTheta = -XDoubleDotD(5)+((Iz-Ix)/Iy)*phiDot*psiDot...
        -JTP*phiDot*wStar/Iy-Kt*L*thetaDot/Iy+Eta(5)*Stheta+K(5)*tanh(Stheta)+FHAT_THETA;
    ...
    TrackingPartPsi = -XDoubleDotD(6)+((Ix-Iy)/Iz)*phiDot*thetaDot-Kt*L*psiDot/Iz...
                              +Eta(6)*Spsi+K(6)*tanh(Spsi)+FHAT_PSI;
